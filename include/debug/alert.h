...
inline char* _make_location_str(char* buf, char const* file, size_t line)
{
  sprintf(buf, "%s:%zu:", strstr(file, "src"), line);
  return buf;
}

inline void _alert_impl(char const* tag, char const* msg, char const* file,
                        size_t line)
{
  char buf[0x100];
  char buf2[0x400]{' '};
  ...
  auto len = sprintf(buf2, COL_BOLD _BRGB(20, 20, 20) "        %s%-30s %s %s",
